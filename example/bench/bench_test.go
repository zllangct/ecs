...
// ============================================================================
// 维度13: ComponentAccessor优化 vs 原始GetBuddy对比
// 使用ComponentAccessor可以避免循环内重复的类型检查和map查找
// ============================================================================

// OptimizedMovementSystem - 使用ComponentAccessor优化的移动系统
type OptimizedMovementSystem struct{}

func (s *OptimizedMovementSystem) Init(ctx *ecs.SystemInitContext) error {
	ctx.SetOption(
		ecs.WithName("OptimizedMovementSystem"),
		ecs.WithDep[components.Transform](ecs.ReadWrite),
		ecs.WithDep[components.Velocity](ecs.ReadOnly),
	)
	return nil
...
func (s *OptimizedMovementSystem) Update(ctx *ecs.SystemContext, event ecs.Event) error {
	deltaSeconds := float32(event.Delta.Seconds())
	if deltaSeconds == 0 {
		deltaSeconds = 1.0 / 60.0
	}

	// 优化：在循环外创建ComponentAccessor，避免循环内重复查找
	transformAccessor := ecs.NewComponentAccessor[components.Transform](ctx)
	velocityAccessor := ecs.NewComponentAccessor[components.Velocity](ctx)
	...
	for index := range query.Iter() {
		// 使用Accessor.Get代替GetBuddy，性能更好
		transform, ok := transformAccessor.Get(index)
		if !ok {
			continue
		}
		velocity, ok := velocityAccessor.Get(index)
		if !ok {
			continue
		}
...
func setupECSWorldOptimized(entityCount int) ecs.World {
	world := ecs.NewWorld(ecs.WithWorldSyncMode())
	world.RegisterStandard(&OptimizedMovementSystem{})
...
func BenchmarkAccessor_Original_10000(b *testing.B) {
	world := setupECSWorldSimple(10000)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		world.Update()
...
func BenchmarkAccessor_Optimized_10000(b *testing.B) {
	world := setupECSWorldOptimized(10000)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		world.Update()
	}
...
func BenchmarkAccessor_Original_100000(b *testing.B) {
	world := setupECSWorldSimple(100000)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		world.Update()
...
func BenchmarkAccessor_Optimized_100000(b *testing.B) {
	world := setupECSWorldOptimized(100000)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		world.Update()
	}
...
// UltraOptimizedMovementSystem - 使用IterIndexOnly的超高性能移动系统
type UltraOptimizedMovementSystem struct{}

func (s *UltraOptimizedMovementSystem) Init(ctx *ecs.SystemInitContext) error {
	ctx.SetOption(
		ecs.WithName("UltraOptimizedMovementSystem"),
		ecs.WithDep[components.Transform](ecs.ReadWrite),
		ecs.WithDep[components.Velocity](ecs.ReadOnly),
	)
	return nil
...
func (s *UltraOptimizedMovementSystem) Update(ctx *ecs.SystemContext, event ecs.Event) error {
	deltaSeconds := float32(event.Delta.Seconds())
	if deltaSeconds == 0 {
		deltaSeconds = 1.0 / 60.0
	}

	// 优化：使用ComponentAccessor
	transformAccessor := ecs.NewComponentAccessor[components.Transform](ctx)
	velocityAccessor := ecs.NewComponentAccessor[components.Velocity](ctx)
	...
	// 超高性能：使用IterIndexOnly，避免获取EntityInfo
	for index := range query.IterIndexOnly() {
		transform := transformAccessor.GetUnsafe(index)
		velocity := velocityAccessor.GetUnsafe(index)
...
func setupECSWorldUltraOptimized(entityCount int) ecs.World {
	world := ecs.NewWorld(ecs.WithWorldSyncMode())
	world.RegisterStandard(&UltraOptimizedMovementSystem{})
...
func BenchmarkAccessor_Ultra_10000(b *testing.B) {
	world := setupECSWorldUltraOptimized(10000)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		world.Update()
	}
...
func BenchmarkAccessor_Ultra_100000(b *testing.B) {
	world := setupECSWorldUltraOptimized(100000)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		world.Update()
	}
