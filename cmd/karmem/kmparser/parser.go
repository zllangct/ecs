...
type Reader struct {
	...
	structFieldTailCommentStart bool
	structFieldFollowEmptyLine  bool
}
...
func (r *Reader) Parser() (d *Content, err error) {
	...
	return nil, fmt.Errorf("error on %s:%d:%d: %s", r.path, r.line+1, r.column, r.error)
}

func (r *Reader) commentInit(f parserFunc) parserFunc {
	if !r.peekEqual("//") {
		r.error = errors.New(`invalid comment, expecting "// content of comment"`)
		return nil
	}

	r.Parsed.Comments = append(r.Parsed.Comments, MultiComment{
		Value: []Comment{{Value: ""}},
	})
	r.Parsed.Blocks = append(r.Parsed.Blocks, Block{
		BlockType: BlockTypeComments,
		Index:     int32(len(r.Parsed.Comments) - 1),
	})

	return r.commentBody(f)
}

func (r *Reader) commentBody(f parserFunc) parserFunc {
	b := r.nextRune()
	switch {
	case b == '\n':
		return f
	...
	r.Parsed.Comments[len(r.Parsed.Comments)-1].Value[0].Value += string(b)
...
func (r *Reader) bodyInit() parserFunc {
	...
	case r.peekEqual("struct") || r.peekEqual("component"):
		return r.structInit

	case r.peekEqual("//"):
		return r.commentInit(r.bodyInit)
...
func (r *Reader) structInit() parserFunc {
	if !r.peekEqual("struct ") && !r.peekEqual("component ") {
		r.error = errors.New(`invalid struct, expecting "struct/component Name [inline | table] {}"`)
	}
	s := Structure{}
	if r.peekEqual("struct ") {
		r.skip(len("struct "))
		s.Data.IsComponent = false
	} else {
		r.skip(len("component "))
		s.Data.IsComponent = true
		s.Data.Class = StructClassInline
		r.Parsed.HasComponent = true
	}
	r.Parsed.Structs = append(r.Parsed.Structs, s)
...
func (r *Reader) structType() parserFunc {
	...
	case r.peekEqual("table"):
		if t.IsComponent {
			r.error = fmt.Errorf(`component must be inline or default to inline`)
			return nil
		}
...
func (r *Reader) structFieldName() parserFunc {
	...
	case unicode.IsNumber(b):
		...
		fallthrough
	case unicode.IsLetter(b):
		if len(f.Name) == 0 && !r.structFieldFollowEmptyLine {
			if len(t.Comments) > 0 {
				f.TopComments.Value = append(f.TopComments.Value, t.Comments[len(t.Comments)-1].Value...)
				t.Comments = t.Comments[:len(t.Comments)-1]
				t.Block = slices.Delete(t.Block, len(t.Block)-2, len(t.Block)-1)
			}
		}
		if len(f.Name) == 0 && unicode.IsLower(b) {
			r.error = fmt.Errorf(`invalid field name, names can't start with lowercase, got %q`, string(b))
...
func (r *Reader) structComment() parserFunc {
	line, ok := r.peekLine()
	if !ok {
		r.error = errors.New(`invalid struct, missing body content`)
	}
	line = bytes.TrimSpace(line)

	t := &r.Parsed.Structs[len(r.Parsed.Structs)-1].Data
	if bytes.HasPrefix(line, []byte("//")) {
		if !r.structFieldTailCommentStart {
			r.structFieldFollowEmptyLine = false
			t.Comments = append(t.Comments, MultiComment{Value: []Comment{}})
			t.Block = append(t.Block, Block{BlockType: BlockTypeFieldComments, Index: int32(len(t.Comments) - 1)})
		}
	} else {
		if len(line) == 0 {
			r.skipLine()
			...
			t.Block = append(t.Block, Block{BlockType: BlockTypeField, Index: int32(len(t.Fields) - 1)})
			return r.structFieldName
		}
	}

	if r.structFieldTailCommentStart {
		r.structFieldTailCommentStart = false
		field := &t.Fields[len(t.Fields)-1].Data
		field.TailComments.Value = string(line)
		r.skipLine()
		return r.structComment
	}

	for bytes.HasPrefix(line, []byte("//")) {
		t.Comments[len(t.Comments)-1].Value = append(t.Comments[len(t.Comments)-1].Value, Comment{string(line)})
		r.skipLine()

		line, ok = r.peekLine()
		if !ok {
			r.error = fmt.Errorf("invalid  empty comment line")
			return nil
		}
		line = bytes.TrimSpace(line)
	}
	...
	if len(line) == 0 {
		r.skipLine()
		r.structFieldFollowEmptyLine = true
		return r.structComment
	} else {
		if bytes.HasPrefix(line, []byte("}")) {
			return r.structEnd
...
func (r *Reader) structFieldType() parserFunc {
	...
	case b == ';':
		...
		if f.Type.PlainSchema == "char" {
			t.Size.Content += 4
		}

		if f.Type.Model == TypeModelSingle && (f.Type.Format == TypeFormatTable || f.Type.Format == TypeFormatStruct) {
			t.Size.Content += _primitives["bool"]
		}
		...
		if r.peekLineStartWith("//") {
			r.structFieldTailCommentStart = true
			return r.structComment
		}
	...
	case b == ']':
		...
		if f.Type.Schema[len(f.Type.Schema)-1] == '[' {
			...
			if t.IsComponent {
				r.error = fmt.Errorf(`slice type is invalid, component field must be array`)
				return nil
			}
		}
...
func (r *Reader) structEnd() parserFunc {
	...
	case b == '}':
		...
		t.Size.Padding = 0
		if !t.Packed {
			t.Size.Padding = (8 - (t.Size.Content % 8)) % 8
		}
		...
		block := Block{
			Index: int32(len(r.Parsed.Structs) - 1),
		}
		if t.IsComponent {
			block.BlockType = BlockTypeComponent
		} else {
			block.BlockType = BlockTypeStruct
		}
		r.Parsed.Blocks = append(r.Parsed.Blocks, block)
...
func (r *Reader) skipLine() {
	line, ok := r.peekLine()
	if !ok {
		r.error = fmt.Errorf(`unexpected line in %s`, line)
		return
	}
	_, err := r.buf.Discard(len(line))
	if err != nil {
		r.error = err
	}
	r.line++
...
func (r *Reader) peekLine() ([]byte, bool) {
	v, err := r.buf.Peek(r.buf.Buffered())
	if err != nil {
		return nil, false
	}
	lineEndIndex := bytes.IndexByte(v, '\n')
	if lineEndIndex >= 0 {
		return v[:lineEndIndex+1], true
	}
	return v[:], true
}

func (r *Reader) peekLineStartWith(prefix string) bool {
	v, ok := r.peekLine()
	if !ok {
		return false
	}
	v = bytes.TrimSpace(v)
	return bytes.HasPrefix(v, []byte(prefix))
}
