...
func TestComponentBasic(t *testing.T) {
	path := "testdata/component_basic.km"
	f, err := os.Open(path)
	if err != nil {
		t.Error(f)
		return
...
func TestStructFieldString(t *testing.T) {
	path := "testdata/structfieldstring.km"
	f, err := os.Open(path)
	if err != nil {
		t.Error(f)
		return
...
func TestRefTag(t *testing.T) {
	path := "testdata/fieldenableref.km"
	f, err := os.Open(path)
	if err != nil {
		t.Error(f)
		return
	...
	if r.Parsed.Structs[1].Data.Tags[0].Name != "golang.field.enable_ref" || r.Parsed.Structs[1].Data.Tags[1].Name != "id" {
		t.Error("invalid tags")
	}
	if r.Parsed.Structs[1].Data.Tags[0].Value != "false" {
		t.Error("invalid tags")
	}
}
