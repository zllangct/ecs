...
func (gen *Golang) functions() (f TemplateFunctions) {
	...
	f.ToTypeSource = func(typ kmparser.Type) string {
		p := f.ToPlainType(typ)
		if typ.PlainSchema == "char" {
			return p
		}
		switch typ.Model {
		case kmparser.TypeModelArray, kmparser.TypeModelSliceLimited:
			switch typ.Format {
			case kmparser.TypeFormatStruct, kmparser.TypeFormatTable:
				return fmt.Sprintf(`[%d]%s`, typ.Length, f.ToPlainType(typ))
			default:
				return fmt.Sprintf(`[%d]%s`, typ.Length, p)
			}
		case kmparser.TypeModelSlice:
			switch typ.Format {
			case kmparser.TypeFormatStruct, kmparser.TypeFormatTable:
				return fmt.Sprintf(`[]%s`, f.ToPlainType(typ))
			default:
				return fmt.Sprintf(`[]%s`, p)
			}
		default:
...
func (gen *Golang) Options() map[string]string {
	return map[string]string{
		"package":          "",
		"import":           "github.com/zllangct/ecs/karmem",
		"field.enable_ref": "false",
		"disposable":       "false",
		"nomadic":          "false",
	}
}
