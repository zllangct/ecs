...
type Writer interface {
	Alloc(n uint) (uint, error)
	WriteAt(offset uint, data []byte)
	Write1At(offset uint, data uint8)
	Write2At(offset uint, data uint16)
	Write4At(offset uint, data uint32)
	Write8At(offset uint, data uint64)
	Reset()
	Bytes() []byte
}

// BytesWriter holds the encoded, the finished encode can be retrieved by Writer.Bytes()
...
func NewWriter(capacity int) Writer {
	return &BytesWriter{Memory: make([]byte, 0, capacity), isFixed: false}
}
...
func (m *Reader) Reset(mem []byte) error {
	if len(mem) == 0 {
		return errors.New("empty buffer")
	}
	m.Memory = mem
	m.Size = 0
	if ok := m.SetSize(uint32(len(mem))); !ok {
